...
{
	if(number>=ZERO && number <= MAX_NUM_TWO_SEV)
	{
		u8 NUMBER1=ZERO;
	    u8 NUMBER2=ZERO;
	    NUMBER1=number%TEN;
	    NUMBER2=(number%ONE_HUNDRED)/TEN ;
	    u8 counter1 = ZERO ;
	    u8 counter2 = ZERO ;

	    for (counter1= LOOP_START;counter1<=NUMBER2;counter1++)
	    {
	    	SEV_COM_ON(SEV_SEGMENT_ONE_COM);
			SEV_SEGMENT_DISPLAY_NUM(counter1);
			_delay_ms(WAIT);
			SEV_COM_OFF(SEV_SEGMENT_ONE_COM);

	    	for(counter2=LOOP_START; counter2<=NINE;counter2++)
	    	{
	    		SEV_COM_ON(SEV_SEGMENT_TWO_COM);
	    		SEV_SEGMENT_DISPLAY_NUM(counter2);
	    		_delay_ms(WAIT);
...
#if   SEV_SEGMENT_COM   ==  GROUP_1
		DDRA|=(ONE<<pin);
#elif SEV_SEGMENT_COM   ==  GROUP_2
		DDRB|=(ONE<<pin);
#elif SEV_SEGMENT_COM   ==  GROUP_3
		DDRC|=(ONE<<pin);
#elif SEV_SEGMENT_COM   ==  GROUP_4
		DDRD|=(ONE<<pin);
#endif
